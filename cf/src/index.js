...
async function answerQuery(question, docs, history, apiKey) {
  const context = docs.length
    ? docs.map((d, i) => `${i + 1}. ${d.content}`).join('\n')
    : '(ไม่มีข้อมูลใน knowledge base)';

  const system = `คุณคือ "น้องมินนี่" ผู้ช่วย Support ทีม Thunder Solution
...
async function chatReply(text, history, docs, apiKey, userProfile = null) {
